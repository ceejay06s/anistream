...
import { getEpisodeServers } from '../services/streamingService.js';
import { resolveEpisodeSources } from '../services/streamResolver.js';
import { gotScraping } from 'got-scraping';

export const streamingRoutes = new Hono();

const normalizeCategory = (value: string | undefined): 'sub' | 'dub' | 'raw' => {
  if (value === 'dub' || value === 'raw') {
    return value;
  }
  return 'sub';
};
...
streamingRoutes.get('/sources', async (c) => {
  ...
  const result = await resolveEpisodeSources({
    episodeId,
    category,
    preferredServer: requestedServer,
    fallback,
  });
